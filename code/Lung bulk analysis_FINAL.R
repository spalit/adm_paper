...
# FIGURE 5
  ...
  data <- data[-which(rowSums(data[,c(2:ncol(data))]) == 0),]
  
  # How to deal with duplicate gene symbols??
  data_sum <- data %>% group_by(gene) %>% summarise(across(everything(), list(median))) %>% as.data.frame()
  ...
  # DElist[which(rownames(DElist) %in% NK_markers_all$GeneSymbol),]

  NK_markers_UP <- NK_markers_all[NK_markers_all$logFC_AMKO_LPS_vs_PBS > 0,]
  # NK_markers_all[NK_markers_all$logFC_interact > 0,] %>% View
  NK_markers_UP_head30 <- NK_markers_UP[order(NK_markers_UP$adj.P.Val_AMKO_LPS_vs_PBS),] %>% head(30)
  
  endo_markers_DOWN <- endo_markers_all[endo_markers_all$logFC_AMKO_LPS_vs_PBS < 0,]
  endo_markers_DOWN_head30 <- endo_markers_DOWN[order(endo_markers_DOWN$adj.P.Val_AMKO_LPS_vs_PBS),] %>% head(30)
  

  type2_markers_DOWN <- type2_markers_all[type2_markers_all$logFC_AMKO_LPS_vs_PBS < 0,]
  type2_markers_DOWN_head30 <- type2_markers_DOWN[order(type2_markers_DOWN$adj.P.Val_AMKO_LPS_vs_PBS),] %>% head(30)
  
  NK_markers_UP_head30$term <- rep('NK',nrow(NK_markers_UP_head30))
  endo_markers_DOWN_head30$term <- rep('Endothelial',nrow(endo_markers_DOWN_head30))
  type2_markers_DOWN_head30$term <- rep('Type 2', nrow(type2_markers_DOWN_head30))
  
  binoy <- rbind.data.frame(NK_markers_UP_head30,endo_markers_DOWN_head30,
                            type2_markers_DOWN_head30)
  
  # convert human and mouse gene symbols from bulk RNA seq data to human symbols to calculate ECDF on BPD data
  require("biomaRt")
  human = useMart("ensembl", dataset = "hsapiens_gene_ensembl", host = "https://dec2021.archive.ensembl.org/")
  mouse = useMart("ensembl", dataset = "mmusculus_gene_ensembl", host = "https://dec2021.archive.ensembl.org/")
  
  genes_hsa = getLDS(attributes = c("mgi_symbol"), filters = "mgi_symbol", values = binoy$GeneSymbol , 
         mart = mouse, attributesL = c("hgnc_symbol"), martL = human, uniqueRows=T)
  
  binoy$gene_hsa <- genes_hsa$HGNC.symbol[match(binoy$GeneSymbol,genes_hsa$MGI.symbol)]
  # binoy[,c(1,12)] %>% View
  binoy$gene_hsa[3] <- 'CCL5'
  
  orig_dataset <- binoy[c(1,5)]
  rownames(orig_dataset) <- orig_dataset$GeneSymbol
  orig_dataset <- orig_dataset[,-1, drop = F]
  
  DElist_sample <- DElist[intersect(rownames(orig_dataset),rownames(DElist)),]
  GEO_dataset <- DElist_sample[,c(1), drop = F]
  orig_dataset <- orig_dataset[GEO_dataset %>% rownames,,drop=F]
  
  all(rownames(orig_dataset) == rownames(GEO_dataset))
  all(rownames(orig_dataset) %in% rownames(GEO_dataset))
  
  a <- GEO_dataset[intersect(rownames(GEO_dataset),NK_markers_UP_head30$GeneSymbol),1]
  b <- orig_dataset[intersect(rownames(orig_dataset),NK_markers_UP_head30$GeneSymbol),1]
  nnn_nk <- data.frame(logFC = c(a,b),group = c(rep('BPD-human',length(a)),rep('BPD-mice',length(b))))
  ggplot(nnn_nk, aes(x=logFC, col=group)) + 
    stat_ecdf() + theme_classic() + ggtitle('NK markers')
  
  a <- GEO_dataset[intersect(rownames(GEO_dataset),endo_markers_DOWN_head30$GeneSymbol),1]
  b <- orig_dataset[intersect(rownames(orig_dataset),endo_markers_DOWN_head30$GeneSymbol),1]
  nnn_endo <- data.frame(logFC = c(a,b),group = c(rep('BPD-human',length(a)),rep('BPD-mice',length(b))))
  ggplot(nnn_endo, aes(x=logFC, col=group)) + 
    stat_ecdf() + theme_classic() + ggtitle('Endothelial markers')
  
  a <- GEO_dataset[intersect(rownames(GEO_dataset),type2_markers_DOWN_head30$GeneSymbol),1]
  b <- orig_dataset[intersect(rownames(orig_dataset),type2_markers_DOWN_head30$GeneSymbol),1]
  nnn_ty <- data.frame(logFC = c(a,b),group = c(rep('BPD-human',length(a)),rep('BPD-mice',length(b))))
  ggplot(nnn_ty, aes(x=logFC, col=group)) + 
    stat_ecdf() + theme_classic() + ggtitle('Type II pneumocyte markers')
  
  
  
Xxx <- lcpm[intersect(rownames(GEO_dataset),NK_markers_UP_head30$GeneSymbol),]
Xxx <- Xxx[,rownames(metadata01)]
my_vec <- Xxx[,grep('C',colnames(Xxx))] %>% rowMeans

# Calculate the mean expression across all control samples and 
# divide entire data matrix by this vector to get log fold change matrix
lfc_mat_NK <- log2(Xxx/my_vec)

# ordering of BPD samples
lfc_mean_NK_BPD <- colMeans(lfc_mat_NK[,grep('B',colnames(lfc_mat_NK))])
xpp <- lfc_mean_NK_BPD[order(lfc_mean_NK_BPD)] %>% names()
# ordering of control samples
lfc_mean_NK_C <- colMeans(lfc_mat_NK[,grep('C',names(colMeans(lfc_mat_NK)))])
xpo <- lfc_mean_NK_C[order(lfc_mean_NK_C)] %>% names()

metadata01 <- metadata
rownames(metadata01) <- metadata01$Sample
metadata01 <- metadata01[order(metadata01$Group),]
metadata01 <- metadata01[,-1,drop = F]

pheatmap(lfc_mat_NK[,c(xpp,xpo)], scale = 'row', annotation_col = metadata01, fontsize_row = 7,fontsize_col = 7,
         border_color = NA, cellheight = 6, cellwidth = 6, 
         cluster_cols = F, breaks = seq(-2, 2, length = 100), gaps_col = 21)


foo <- cbind.data.frame(group = metadata$Group, 
                        expr = lcpm[which(rownames(lcpm) == 'Rab37'),])
p <- foo %>%
  ggplot( aes(x=group, y=expr, fill=group)) +
  geom_boxplot() +
  scale_fill_viridis(discrete = TRUE, alpha=0.6) +
...
tiya <- melt(Xxx)
tiya$Var2 <- gsub('[[:digit:]]','',tiya$Var2)
colnames(tiya) <- c('gene','group','expr')
ggplot(tiya, aes(x=gene, y=expr, fill=group)) + 
  geom_boxplot() + theme_classic() + 
  theme(axis.text.x = element_text(angle = 90)) + facet_wrap(~gene, scale="free")